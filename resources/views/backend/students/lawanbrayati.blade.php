...
<table class="table text-center" >
    ...
    @if($unp->fees > $unp->paid && $unp->statue == 'active')
    <tr style="text-align:center">
        <td><a href="{{ route('student.show',$unp->id) }}" style="color: #df0000">
        {{$unp->fullname}}
                        </a>@if($unp->note != Null)
        ...
        <td>{{ $unp->fees }}</td>
        <td>{{ $unp->paid }}</td>
        <td>{{$unp->fees - $unp->paid }}</td>
        <td>{{$unp->last_payment_date}}</td>
        <td>                        <a href="{{ route('student.edit',$unp->id) }}" style="float:left ;" title="Edit">
