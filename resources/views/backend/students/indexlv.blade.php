...
@section('content')
    <div class="roles-permissions">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-gray-700 uppercase font-bold">Lawan Vocational Students</h2>
            </div>
        ...
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300" style="text-align: center">
            <div class="flex flex-wrap items-center uppercase text-sm font-semibold bg-gray-600 text-white rounded-tl rounded-tr">
                ...
                <div class="w-1/12 px-4 py-3">Mother</div>
                <div class="w-2/12 px-4 py-3">Birth</div>
                <div class="w-1/12 px-4 py-3">Phone</div>
                <div class="w-2/12 px-4 py-3">Course</div>
                <div class="w-1/12 px-4 py-3">Fees</div>
                <div class="w-1/12 px-4 py-3">paid</div>
            ...
            @foreach ($students as $student)
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300" style="text-align: center">
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $student->first_name }} {{$student->father_name}} {{$student->grandfather_name}}</div>
                    <div class="w-1/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $student->mother }}</div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $student->dateofbirth }}-{{ $student->placeofbirth }}</div>
                    <div class="w-1/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $student->phone1 }} {{ $student->phone2 }}</div>
                    ...
                    <div class="w-1/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $student->fees }} </div>
                    <div class="w-1/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $student->paid }} </div>
                    <div class="w-2/12 flex items-center justify-end px-3">
                        <a href="{{ route('paymentpays.create1',$student->id) }}" class="ml-1 bg-green-600 block p-1 border border-blue-600 rounded-sm" title="View Profile">
                            PAID
                        </a>
                        <a href="{{ route('paymentdetails.create1',$student->id) }}" class="ml-1 bg-red-600 block p-1 border border-blue-600 rounded-sm" title="View Profile">
                            PAYS
                        </a>
