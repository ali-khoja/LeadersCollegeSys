...
@section('content')
    <div class="roles-permissions">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-gray-700 uppercase font-bold" style="justify-content: center">المدرسين</h2>
            </div>
            <div class="flex flex-wrap items-center">
                <a href="{{ route('teacher.create') }}" class="bg-green-500 text-white text-sm uppercase py-2 px-4 flex items-center rounded">
                    ...
                    <span class="ml-2 text-xs font-semibold">إضافة مدرس</span>
                </a>
            </div>
        </div>
        ...
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300" style="text-align: center">
            <div class="flex flex-wrap items-center uppercase text-sm font-semibold bg-gray-600 text-white rounded-tl rounded-tr">
                <div class="w-3/12 px-4 py-3">الاسم</div>
                <div class="w-2/12 px-4 py-3">رقم الهاتف</div>
                <div class="w-1/12 px-4 py-3">الجنس</div>
                <div class="w-4/12 px-4 py-3">القسم</div>
                <div class="w-2/12 px-4 py-3 text-right">خيارات</div>
            </div>
            @foreach ($teachers as $teacher)
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300" style="text-align: center">
                    <div class="w-3/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $teacher->name }}</div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $teacher->phone }}</div>
                    <div class="w-1/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $teacher->gender }}</div>
                    <div class="w-4/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $teacher->section }}</div>
                    <div class="w-2/12 flex items-center justify-end px-3">
                                               <a href="{{ route('teacher.show',$teacher->id) }}" title="show" style="float:left">
