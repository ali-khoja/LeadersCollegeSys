            ...
            <form action="{{ route('profile.update') }}" method="POST" class="w-full max-w-2xl mx-auto px-6 py-12 flex flex-wrap justify-between" enctype="multipart/form-data">
                ...
                <div class="w-full sm:w-auto order-last sm:order-first">
                    ...
                    <div class="md:flex md:items-center mb-4">
                        ...
                        <div class="md:w-2/3">
                            <input name="password" class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500" type="password" value="{{ auth()->user()->password }}">
                            @error('password')
                                <p class="text-red-500 text-xs italic">{{ $message }}</p>
                            @enderror
                        </div>
